...
void dosplit(dch *in, dch2 *out, eb_chan wait) {
    ...
    const void *dat;
    assert(eb_chan_recv(in->dat, &dat) == eb_chan_res_ok);
    eb_chan_send((*out)[0]->dat, dat);
	if (!both) {
        assert(eb_chan_recv(wait, NULL) == eb_chan_res_ok);
	}
    
    assert(eb_chan_recv((*out)[1]->req, NULL) == eb_chan_res_ok);
...
void put(rat dat, dch *out) {
    assert(eb_chan_recv(out->req, NULL) == eb_chan_res_ok);
...
rat get(dch *in) {
    ...
    rat *v;
    assert(eb_chan_recv(in->dat, (void*)&v) == eb_chan_res_ok);
    return heap2rat(v);
}
...
PS Add(PS U, PS V) {
	...
	go(
		rat *uv;
		for (;;) {
            assert(eb_chan_recv(Z->req, NULL) == eb_chan_res_ok);
			uv = get2(U,V);
