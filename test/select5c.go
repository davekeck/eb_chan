...
var recv = parse("recv", `
	...
	{{else}}
        {{if .Maybe}}
            timeout = eb_nsec_zero;
        {{else}}
            timeout = eb_nsec_forever;
        {{end}}
        
    	{
            ...
            eb_chan_op op6 = eb_chan_op_send(dummy, 1);
            eb_chan_op op7 = eb_chan_op_recv(dummy);
            eb_chan_op op8 = eb_chan_op_send(nilch, 1);
            eb_chan_op op9 = eb_chan_op_recv(nilch);
            
            eb_chan_op *r = eb_chan_do(timeout,
                {{if .Maybe}}
            	    &op1,
            	{{else}}
                    {{if .Maybe}}
