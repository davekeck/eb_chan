...
void chanchan() {
    ...
    const void *val;
    assert(eb_chan_recv(cc, &val) == eb_chan_res_ok);
    
    assert(eb_chan_try_send((eb_chan)val, (void*)2) == eb_chan_res_ok);
    
    const void *val2;
    assert(eb_chan_recv(c, &val2) == eb_chan_res_ok);
...
void sendprec() {
    ...
    const void *val;
    assert(eb_chan_recv(c, &val) == eb_chan_res_ok);
    assert(val);
}
