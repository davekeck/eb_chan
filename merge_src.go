...
/* A naive parser for quoted #includes/#imports. Can be tricked if it's inside a C++-style comment. */
func getIncPath(line string) string {
    var kIncStmts = [...]string{"#include", "#import"}
    tline := strings.TrimSpace(line)
    for _, incStmt := range kIncStmts {
        if !strings.HasPrefix(tline, incStmt) {
            continue
        }
        
        parts := strings.Split(tline, `"`)
        if len(parts) < 3 || strings.TrimSpace(parts[0]) != incStmt {
            continue
        }
